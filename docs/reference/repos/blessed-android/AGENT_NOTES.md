# Agent notes for weliem/blessed-android
...
```bash
grep -R "BluetoothHidDevice\|registerApp\|sendReport\|BluetoothProfile.HID_DEVICE" -n docs/reference/repos/blessed-android/src | head -100
grep -R "00001812\|HID_SERVICE\|Report Map\|ScanSettings\|setLegacy\|setPhy" -n docs/reference/repos/blessed-android/src | head -100
```
